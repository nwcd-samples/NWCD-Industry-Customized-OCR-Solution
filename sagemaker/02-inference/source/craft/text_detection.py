...
def text_detection_single(image_file, net, args, output_dir):
    
    #output_dir = '/'.join(image_file.split("/")[0:-1])
    ...
    try:      
        ...
        image = craft.imgproc.loadImage(image_file)
        bboxes, polys, score_text = test_net(args, net, image, args.text_threshold, args.link_threshold, args.low_text, args.cuda, args.poly, refine_net)

        # save score text
        filename, file_ext = os.path.splitext(os.path.basename(image_file))

        craft.file_utils.saveResult(image_file, image[:,:,::-1], polys, dirname=output_dir, write_image=False)
    except Exception:
        print("【Error】 图片[{}]  文本区域检测识别失败".format(image_file))
